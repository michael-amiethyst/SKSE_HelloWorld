...
{
    CHECK(std::string_view(NotificationService::GetMessage_(OpenVRTransportDetector::Transport::kWired)) ==
          "Zenithar's courier travels through the earth.");
}

TEST_CASE("NotificationService selects the wireless transport message")
{
    CHECK(std::string_view(NotificationService::GetMessage_(OpenVRTransportDetector::Transport::kWireless)) ==
          "Zenithar's courier travels through the air.");
}

TEST_CASE("NotificationService selects the fallback transport message")
{
    CHECK(std::string_view(NotificationService::GetMessage_(OpenVRTransportDetector::Transport::kUnknown)) ==
